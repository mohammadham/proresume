<script>
    ...
    // var curr_url = "{{ url()->current() . '?language=' }}";
     var curr_url = "{{ url()->current() }}"; // Current URL without query parameters

</script>

<script>
    let search = "{{ __('Search:') }}";
    let lengthMenu = "{{ __('Show') }} _MENU_ {{ __('Entries') }}";
    let info = "{{ __('Showing') }} _START_ {{ __('to') }} _END_ {{ __('of') }} _TOTAL_ {{ __('Entries') }}";
    let nextForDatatable = "{{ __('Next') }}";
    let previousForDatatable = "{{ __('Previous') }}";
    let warningMsg = "{{ __('Warning') }}";
    let successMsg = "{{ __('Success') }}";
</script>
...
<script>
    ...
    let $delete_subtitle = "{{ __('You will not be able to revert this!') }}";
