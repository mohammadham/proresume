                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 mt-2 mt-lg-0">
                            ...
                            <form action="{{ url()->full() }}" class="float-lg-right float-none">
                                <input type="hidden" name="language" value="{{ @$default->code }}">
                                                        ...
                                                        <div class="dropdown">
                                                            ...
                                                            <div class="dropdown-menu"
                                                                ...
                                                                <a class="dropdown-item"
                                                                    href="{{ route('register.user.view', $user->id) . '?language=' . $default->code }}">{{ __('Details') }}</a>
                                                                <a class="dropdown-item"
                                                                    href="{{ route('register.user.changePass', $user->id) . '?language=' . $default->code }}">{{ __('Change Password') }}</a>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('register.user.store') }}" method="POST" id="ajaxForm">
                        ...
                        <div class="form-group">
                            ...
                            <p id="errpassword_confirmation" class="text-danger mb-0 em"></p>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="package_id" class="form-control">
                                @if (!empty($packages))
                                    @foreach ($packages as $package)
                                        <option value="{{ $package->id }}">{{ __($package->title) }} ({{ __($package->term) }})
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                ...
                <div class="modal-footer text-center">
                    <button id="" data-form="ajaxForm" type="button"
                        class="submitBtn btn btn-primary">{{ __('Add User') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $(".template-select").on('change', function() {
                ...
                $(`#templateModal${userId} .text-danger`).text('');
            ...
            @if ($errors->any())
                let userId = {{ old('user_id') }}; // Ensure you have the correct user ID
                $("#templateModal" + userId).modal('show');
                 $(`#templateModal${userId} input[name='template']`).val("{{ old('template') }}");
            @endif
        });
    </script>
@endsection
