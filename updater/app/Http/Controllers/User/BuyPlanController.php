...
class BuyPlanController extends Controller
{
    public function index()
    {
        ...
        if (session()->has('userDashboardLang')) {
            $currentLang = UserLanguage::where([
    ...
    public function checkout($package_id)
    {
        ...
        if($hasPendingMemb) {
            Session::flash('warning', __('You_already_have_a_Pending_Membership_Request') . '.');
            return back();
        }
        if($packageCount >= 2) {
            Session::flash('warning', __('activate_your_next_package_after_current_package_expires') . '.');
            return back();
        }
        $userId = Auth::guard('web')->user()->id;
