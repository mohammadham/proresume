...
class IdPayController extends Controller
{
    ...
    public function successPayment($request)
    {
        ...
        try {
            $transaction = DB::transaction(function () use ($payment_id) {
                $t = Transaction::where('transaction_id', $payment_id)
                    ->whereHas('gateway', function($q) { $q->where('keyword', 'idpay'); })
                    ->lockForUpdate()
                    ->first();
                if (!$t) {
            ...
            if ($msg === 'TRANSACTION_NOT_FOUND') {
                Log::channel('payment')->warning('IDPay callback: transaction not found', ['payment_id' => $payment_id]);
                return redirect($cancel_url)->with('error', 'شناسه پرداخت نامعتبر است.');
            }
            Log::channel('payment')->info('IDPay callback: duplicate/already processed', ['payment_id' => $payment_id, 'state' => $msg]);
            ...
            try {
                ...
                if (isset($result['status']) && $result['status'] == '100') {
                    ...
                    if ($verifiedAmount !== null && $verifiedAmount !== $expectedAmount) {
                        Log::channel('payment')->warning('IDPay amount mismatch', [
                            'payment_id' => $payment_id,
