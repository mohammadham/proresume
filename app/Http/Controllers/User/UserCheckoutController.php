...
class UserCheckoutController extends Controller
{
    public function checkout(ExtendRequest $request)
    {
        ...
        if (session()->has('userDashboardLang')) {
            $currentLang = Language::where([['code', session()->get('userDashboardLang')]])->first();
            session()->put('currentLangCode', session()->get('userDashboardLang'));
        } else {
            $currentLang = UserLanguage::where([['is_default', 1], ['user_id', $userId]])->first();
            session()->put('currentLangCode', $currentLang->code);
        }
        ...
        $description = __('Congratulation you are going to join our membership.Please make a payment for confirming your membership now') . '!';
        if ($request->price == 0) {
            $request['price'] = 0.0;
            ...
            $body = __('You made a payment. This is a confirmation mail from us. Please see the invoice attachment below');
            ...
            if ($be->base_currency_text != 'INR') {
                session()->flash('warning', $be->base_currency_text . ' ' . __('is not allowed for') . ' ' . __('Razorpay'));
                return back()->withInput($request->all());
            }
            ...
            if ($be->base_currency_text != 'INR') {
                session()->flash('warning', $be->base_currency_text . ' ' . __('is not allowed for') . ' ' . __('Instamojo'));
                return back()->withInput($request->all());
            }
            if ($request->price < 9) {
            ...
            if (!in_array($be->base_currency_text, $available_currency)) {
                session()->flash('warning', $be->base_currency_text . ' ' . __('is not allowed for') . ' ' . __('Flutterwave'));
                return back()->withInput($request->all());
            }
            $amount = $request->price;
            $email = $request->email;
            ...
            if (!in_array($be->base_currency_text, $available_currency)) {
                session()->flash('warning', $be->base_currency_text . ' ' . __('is not allowed for') . ' ' . __('Mollie'));
                return back()->withInput($request->all());
            }
            $amount = $request->price;
    ...
    public function store($request, $transaction_id, $transaction_details, $amount, $be, $password)
    {
        ...
        $previousMembership = Membership::query()
            ...
            ->where([['user_id', $user->id], ['start_date', '<=', Carbon::now()->toDateString()], ['expire_date', '>=', Carbon::now()->toDateString()]])
            ->where('status', 1)
