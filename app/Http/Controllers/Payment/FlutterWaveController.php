...
class FlutterWaveController extends Controller
{
    ...
    public function paymentProcess(Request $request, $_amount, $_email, $_item_number, $_successUrl, $_cancelUrl, $bex)
    {
        ...
        if (isset($transaction->data) && isset($transaction->data->link)) {
            return redirect()->to($transaction->data->link);
        } else {
            // there was an error from the API
            return redirect($cancel_url)->with('error', 'API returned error: ' . ($transaction->message ?? 'Unexpected response structure'));
        }
